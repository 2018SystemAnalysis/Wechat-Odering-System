...
Page({
  ...
  selectInfo2: function (e) {
    var type = e.currentTarget.dataset.type;
    var index = e.currentTarget.dataset.index;
    var a = this.data;
    var tem = a.listData[type].foods[index].tem;
    var temBox = [];
    var name = a.listData[type].foods[index].name;
    for (let i = 0; i < tem.length; i++) {
      temBox.push(tem[i].specs)
    }
    this.setData({
    ...
    var that=this;
    wx.request({
      url: app.globalData.apiHost + '/getcomment?food_name='+name, 
      method: 'GET',
      //data: userInfo,
      //dataType: 'json',
      header: {
      ...
      success: function (res) {
        ...
        let commentsData=[name]
        // for (let i = 0; i < res.data.data.length; i++){
        //   commentsData.push(res.data.data[i].content)
        // }
        that.setData({
